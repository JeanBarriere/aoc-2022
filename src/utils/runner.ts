...
const fetchInput = async (day: number) => {
  ...
  return fetch(inputUrl, { headers: { cookie: `session=${SESSION}` } }).then((res) => res.text());
};
...
export const createAdventRunnerForDay = (day: number) => {
  ...
  return {
    run: <Input, Output>(process: Executor<Input, Output>, transformer?: Transformer<Input>) => {
      describe(`Advent of code Day ${day}`, async () => {
        const result: Output = await input.then(transformer ?? (noTransformer as Transformer<Input>)).then(process);
